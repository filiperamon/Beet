    ...
    class LibVpxEnc : LibVpxCom
    {
            ...
            byte[] frame, int fmt, long frameStart, long frameDuration, long flags, long deadline)
        {
            ...
            try
            {
                ...
                throwOnError();
                return (byte[])JNIEnv.GetArray(Java_com_google_libvpx_LibVpxEnc_vpxCodecEncGetCxData(JNIEnv.Handle, IntPtr.Zero, vpxCodecIface), JniHandleOwnership.TransferLocalRef, typeof(byte));
            }
            finally
            {
